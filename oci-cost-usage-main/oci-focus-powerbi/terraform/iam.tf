...
resource "oci_identity_policy" "focus_function" {
  ...
  statements = [
    ...
    "Allow dynamic-group ${oci_identity_dynamic_group.focus_function.name} to manage buckets in compartment id ${var.compartment_ocid} where all {target.bucket.name = '${oci_objectstorage_bucket.focus.name}', request.permission = 'PAR_MANAGE'}",
